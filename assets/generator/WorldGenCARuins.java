...
public final class WorldGenCARuins extends WorldGeneratorThread {
	...
	public boolean generate(int i0, int j0, int k0) {
		if (caRule == null) //if we haven't picked in an earlier generate call 
			caRule = ((PopulatorCARuins) master).pick(random);
		if (caRule == null)
			return false;
        int th = MinHeight + random.nextInt(MaxHeight - MinHeight + 1);
		int seedCode = RandomPicker.pickWeightedOption(random, seedTypeWeights);
		...
		? BuildingCellularAutomaton.makeSymmetricSeed(Math.min(ContainerWidth, ContainerLength), SymmetricSeedDensity, random) : seedCode == 1 ? BuildingCellularAutomaton.makeLinearSeed(
				ContainerWidth, random) : seedCode == 2 ? BuildingCellularAutomaton.makeCircularSeed(Math.min(ContainerWidth, ContainerLength), random) : BuildingCellularAutomaton
				.makeCruciformSeed(Math.min(ContainerWidth, ContainerLength), random);
