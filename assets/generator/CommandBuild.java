...
public final class CommandBuild extends CommandBase {
	...
	public void processCommand(ICommandSender var1, String[] coordinate) {
		if (coordinate.length > 2) {
			...
			if(world!=null) {
				WorldGeneratorThread wgt = null;
				long worldSeed = world.getSeed();
				Random random = new Random(worldSeed);
				long xSeed = random.nextLong() >> 2 + 1L;
				long zSeed = random.nextLong() >> 2 + 1L;
				random.setSeed((xSeed * (posX / 16) + zSeed * (posZ / 16)) ^ worldSeed);
				if ("ruin".equalsIgnoreCase(coordinate[0])) {
					wgt = (new WorldGenCARuins(PopulatorCARuins.instance, world, random, posX, posZ, 10, 1));
				} else if ("wall".equalsIgnoreCase(coordinate[0])) {
					wgt = (new WorldGenGreatWall(PopulatorGreatWall.instance, world, random, posX, posZ, 10, 1));
				} else if ("city".equalsIgnoreCase(coordinate[0])) {
					wgt = (new WorldGenWalledCity(PopulatorWalledCity.instance, world, random, posX, posZ, 100, 1));
				} else if ("undcity".equalsIgnoreCase(coordinate[0])) {
					PopulatorWalledCity city = PopulatorWalledCity.instance;
					wgt = new WorldGenUndergroundCity(city, world, random, posX, posZ, 10, 1);
				...
				if(wgt!=null){
					wgt.run();
					func_152373_a(var1, this, "/build command used by " + var1.getCommandSenderName());
					return;
				}
			}
		}
